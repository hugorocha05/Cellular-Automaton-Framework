...
pygame.display.set_caption("Cellular Automoton Framework")
...
def show_grid(matrix, border=True):
    ...
    for i in range(len(matrix)):
        for j in range(len(matrix[i])):
            ...
            if border:
                pygame.draw.rect(screen, (50, 50, 50), rect_obj, width=1)
...
def interactive_setup(rows, cols):
...
def run_sim(matrix, neighbors, birth_conditions, survival_conditions, border=True):
    running = True
    paused = False
    step = False
    ...
    while running:
        ...
        if not paused or (paused and step):
            matrix = update_grid(matrix, neighbors, birth_conditions, survival_conditions)
            step = False
...
matrix = interactive_setup(70, 70)
...
if matrix:
    run_sim(matrix=matrix, neighbors=neighbors, birth_conditions=birth_conditions, survival_conditions=survival_conditions, border=True)
